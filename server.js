...
(() => {
  ...
  io.on("connection", socket => {
    ...
    socket.on("cursorMove", data => {
      ...
      var ang = Math.atan2(
        data.y - playerList[id].y,
        data.x - playerList[id].x
      );
    ...
    socket.on("keyPress", data => {
      ...
      playerList[id].dy = 0;
      if (keyArray[0]) {
        if (playerList[id].y >= 32 / 2)
          playerList[id].dy = -playerList[id].vel / Math.sqrt(2);
      }
      if (keyArray[1]) {
        if (playerList[id].x >= 32 / 2)
          playerList[id].dx = -playerList[id].vel / Math.sqrt(2);
      }
      if (keyArray[2]) {
        if (playerList[id].y <= HEIGHT - 32 / 2)
          playerList[id].dy = playerList[id].vel / Math.sqrt(2);
      }
      if (keyArray[3]) {
        if (playerList[id].x <= WIDTH - 32 / 2)
          playerList[id].dx = playerList[id].vel / Math.sqrt(2);
      }
      //shoot key
      if (keyArray[4]) {
  ...
  setInterval(() => {
    ...
    for (var proj in projectileList) {
      ...
      for (var player in playerList) {
        ...
        var py = playerList[player].y;
        if (
          Math.abs(bx - px) < 16 + projectileList[proj].dim / 2 &&
          Math.abs(by - py) < 16 + projectileList[proj].dim / 2
        ) {
          playerList[player].hp -= projectileList[proj].dmg;
          if (playerList[player].hp <= 0) {
            //transfer ammo to killer
            playerList[projectileList[proj].parent].ammo +=
              playerList[player].ammo;
          }
        }
      }
      for (var enemy in enemyList) {
        ...
        var ey = enemyList[enemy].y;
        if (
          Math.abs(bx - ex) < 25 + projectileList[proj].dim / 2 &&
          Math.abs(by - ey) < 25 + projectileList[proj].dim / 2
        ) {
