...
setInterval(() => {
    ...
    for(var player in cPlayerList) {
        //draw
        ctx.fillRect(cPlayerList[player].x-16, cPlayerList[player].y-16, 32, 32);
    }
    //handle projectiles
    ctx.fillStyle = 'blue';
    for(var proj in cProjectileList) {
    ...
    for(var proj = projectiles.length-1; proj >= 0; proj--) {
        ...
        //draw
        ctx.fillRect(projectiles[proj].x - 3, 
        projectiles[proj].y - 3, 6, 6);
    }
